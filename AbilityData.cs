...
namespace TcgEngine
{
    ...
    public class AbilityData : ScriptableObject
    {
        ...
        [Header("Element and Damage")]
        public Element damage_type;
        public Element damage_type_2;
        public AttackPower base_damage;
        ...
        public bool half_action;
        public bool free_action;
        public bool charge_player_action;       // should require player action instead of card action
        public AbilityType ability_type;         //Skills use HP and Spells use MP

        [Header("Activated Ability - Legacy")]
        public bool exhaust;                    //Action cost for activated abilities
        public bool fromDiscard;                //Ability can be activated from the discard pile.
        public bool fromExtraDeck;              //Ability can be activated from the extra deck.
        public bool fromBoard;                  //Ability can be activated from the board.
        ...
        public bool CanTarget(Game data, Card caster, Card target)
        {
            ...
            bool targettingEnemy = caster.player_id != target.player_id;
            bool targetIsProtected = target.HasStatus(StatusType.Protected);

            if (targettingEnemy && targetIsProtected)
                return false; // Cannot target a protected opponent
    ...
    public enum AbilityTrigger
    {
        ...
        OnRemovedFromPlay = 43, //When removed from play

        OnSentToDiscard = 44, //When sent directly to the discard pile
        OnSentToDiscardFromDeck = 45, //When sent to the discard pile from the deck

        OnBeforePlayOther = 46, //Counterspell timing
        OnBeforePlaySelf = 47, //Pre-cast timing, after counterspell timing

        OnOpponentDraw = 48,    //When the opponent draws a card
        OnPlayerDraw = 49,      //When the player draws a card
    }
