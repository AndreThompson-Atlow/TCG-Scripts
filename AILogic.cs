...
namespace TcgEngine.AI
{
    ...
    public class AILogic
    {
        ...
        public int actions_per_turn = 3;          //AI wont predict more than this number of sequential actions per turn, if more than that will EndTurn (Do A, then do B, then do C, then end turn)
        public int actions_per_turn_wide = 7;     //Same but in wide depth
        ...
        public int nodes_per_action_wide = 5;    //Same but in wide depth
        ...
        private AIAction best_action = null;
        ...
        public static AILogic Create(int player_id, int level)
        {
            ...
            job.heuristic = new AIHeuristic(player_id, 10);
        ...
        public void ExecuteLogicalActions(AIType playstyle)
        {
            // Set Type
            bool isAggro = playstyle == AIType.Aggro;
            bool isBalanced = playstyle == AIType.Balance;
            bool isControl = playstyle == AIType.Control;

            // Get Initial Heuristics

            first_node = CreateNode(null, null, ai_player_id, 0, 0);
            first_node.hvalue = heuristic.CalculateHeuristic(original_data, first_node, game_logic);
            first_node.alpha = int.MinValue;
            first_node.beta = int.MaxValue;

            // Create Action List

            Player player = original_data.GetPlayer(original_data.current_player);
            List<AIAction> action_list = list_pool.Create();

            // Select Scenarios

            if (original_data.selector != SelectorType.None)
            {
                AddAndFilterSelectActions(action_list);
            }
            ...
            if (original_data.selector == SelectorType.None)
            {
                ...
                // Phase 1 Actions [Playing Spells/Equipment]
                if(isAggro)
                {
                    // Add units first for aggro decks
                    AddPhase3Actions(action_list, player, isAggro, oneOnly);
                } 

                if(isBalanced)
                {
                    // Try to play a unit or relic first
                    oneOnly = true;
                    AddPhase3Actions(action_list, player, isAggro, oneOnly);
                }

                AddPhase1Actions(action_list, player);
                
                // Phase 2 Actions [Attacking, Using Abilities]
                AddPhase2Actions(action_list, player, isAggro);

                // Phase 3 Actions [Play Units/Relics]
                AddPhase3Actions(action_list, player, isAggro, oneOnly);

                // Second Attack/Ability Phase (For Accelerated Units & Relic Abilities)
                AddPhase2Actions(action_list, player, isAggro);

                // Phase 4 Actions [Fusions]
                AddPhase4Actions(action_list, player);

                // Third Attack/Ability Phase (For Accelerated Fusion Units)
                AddPhase2Actions(action_list, player, isAggro);
            }

            // Set Best Action
            best_action = getCurrentBestAction(original_data, action_list);
        ...
        private void AddAndFilterSelectActions(List<AIAction> action_list)
        {
            AddSelectActions(action_list, original_data, first_node);
            FilterSelectActions(original_data, first_node, action_list);
        }

        // Casting spells and playing equipment
        private void AddPhase1Actions(List<AIAction> action_list, Player player)
        {
            if (!ShouldAddNextPhase(action_list))
                return;

            for (int c = 0; c < player.cards_hand.Count; c++)
            {
                Card card = player.cards_hand[c];
                AddActions(action_list, original_data, first_node, GameAction.PlayCard, card);
            }

            FilterPhase1Actions(original_data, first_node, action_list);
        }

        private bool ShouldAddNextPhase(List<AIAction> action_list)
        {
            AIAction currentBestAction = getCurrentBestAction(original_data, action_list);
            return currentBestAction.type == GameAction.EndTurn;
        }

        // Attacking and using activated abilities
        private void AddPhase2Actions(List<AIAction> action_list, Player player, bool isAggro)
        {
            if (!ShouldAddNextPhase(action_list))
                return;
            ...
            for (int c = 0; c < player.cards_board.Count; c++)
            {
                ...
                AddActions(action_list, original_data, first_node, GameAction.Attack, card);
                AddActions(action_list, original_data, first_node, GameAction.AttackPlayer, card);
                AddActions(action_list, original_data, first_node, GameAction.CastAbility, card, 1);
            }

            //Discard Abilities
            for (int c = 0; c < player.cards_discard.Count; c++)
            {
                Card card = player.cards_discard[c];
                AddActions(action_list, original_data, first_node, GameAction.CastAbility, card, 2);
            }

            FilterPhase2Actions(original_data, first_node, action_list, isAggro);
        }

        private void AddPhase3Actions(List<AIAction> action_list, Player player, bool isAggro, bool oneActionOnly)
        {
            if (!ShouldAddNextPhase(action_list))
                return;

            string lastPlayedCardId = original_data.last_played;
            Card lastPlayedCard = original_data.GetCard(lastPlayedCardId);
            bool lastCardIsUnitOrArt = false;

            if (lastPlayedCard != null)
            {
                CardType lastCardType = lastPlayedCard.CardData.type;
                lastCardIsUnitOrArt = lastCardType == CardType.Character || lastCardType == CardType.Artifact;
            }

            if (lastCardIsUnitOrArt && oneActionOnly)
                return;
            ...
            FilterPhase3Actions(original_data, first_node, action_list);
        }

        private void AddPhase4Actions(List<AIAction> action_list, Player player)
        {
            if (!ShouldAddNextPhase(action_list))
                return;

            //Play Fusion Unit
            for (int c = 0; c < player.cards_extra.Count; c++)
            {
                Card card = player.cards_extra[c];
                AddActions(action_list, original_data, first_node, GameAction.CastAbility, card, 3);
            }

            FilterPhase4Actions(original_data, first_node, action_list);
        }
        ...
        private void CalculateNode(Game data, NodeState node)
        {
            ...
            if (node.taction < max_actions)
            {
                ...
                {
                    //Play card
                    // Set to keep track of card_ids that have been added
                    HashSet<string> addedCardIds = new HashSet<string>();
                    for (int c = 0; c < player.cards_hand.Count; c++)
                    {
                        Card card = player.cards_hand[c];
                        // Check if the card_id has already been added
                        if (!addedCardIds.Contains(card.card_id))
                        {
                            // Add this card_id to the set of added card_ids
                            addedCardIds.Add(card.card_id);

                            // Add the action for this card
                    ...
                    //Fusion Deck
                    // Set to keep track of card_ids that have been added
                    HashSet<string> addedFusionCardIds = new HashSet<string>();
                    for (int c = 0; c < player.cards_extra.Count; c++)
                    {
                        Card card = player.cards_extra[c];
                        // Check if the card_id has already been added
                        if (!addedFusionCardIds.Contains(card.card_id))
                        {
                            // Add this card_id to the set of added card_ids
                            addedFusionCardIds.Add(card.card_id);

                            // Add the action for this card
                            AddActions(action_list, data, node, GameAction.CastAbility, card, 3);
                        }
                    }
                    ...
                    for (int c = 0; c < player.cards_board.Count; c++)
                    {
                        ...
                        AddActions(action_list, data, node, GameAction.CastAbility, card, 1);
                    ...
                    {
                        //Card card = player.cards_discard[c];
                        //AddActions(action_list, data, node, GameAction.CastAbility, card, 2);
                    }
        ...
        private void FilterSelectActions(Game data, NodeState node, List<AIAction> action_list)
        {
            for (int o = 0; o < action_list.Count; o++)
            {
                AIAction action = action_list[o];
            ...
            calculateActionScores(data, action_list);
        }

        private void FilterPhase1Actions(Game data, NodeState node, List<AIAction> action_list)
        {
            for (int o = 0; o < action_list.Count; o++)
            {
                AIAction action = action_list[o];
                ...
                bool is_equipment = card.CardData.type == CardType.Equipment;
                bool is_phase_1_card = is_spell || is_equipment;

                action.valid = action.type == GameAction.PlayCard && is_phase_1_card;
            }

            calculateActionScores(data, action_list);
        }

        private bool isValidAttackPlayerAction(AIAction action)
        {
            bool isAttackAction = action.type == GameAction.AttackPlayer;
            //if (isAttackAction)
                //return isActionBeneficial(original_data, first_node, action);

            return isAttackAction;
        }
        private void FilterPhase2Actions(Game data, NodeState node, List<AIAction> action_list, bool isAggro)
        {
            bool hasValidAttackPlayerAction = action_list.Any(action => isValidAttackPlayerAction(action));
            bool attackPlayerOnly = hasValidAttackPlayerAction && isAggro;
            ...
            for (int o = 0; o < action_list.Count; o++)
            {
                ...
                if (attackPlayerOnly && action.type != GameAction.AttackPlayer)
                    action.valid = false;
            }
        ...
        private void FilterPhase3Actions(Game data, NodeState node, List<AIAction> action_list)
        {
            for (int o = 0; o < action_list.Count; o++)
            {
                AIAction action = action_list[o];
                ...
                Card card = data.GetCard(action.card_uid);
                bool isBoardCard = card != null && card.CardData.IsBoardCard();
                bool is_equipment = card.CardData.type == CardType.Equipment;

                if (isBoardCard && !is_equipment)
                    action.valid = true;
                else
                    action.valid = false;
            }
        ...
        private void FilterPhase4Actions(Game data, NodeState node, List<AIAction> action_list)
        {
            for (int o = 0; o < action_list.Count; o++)
            {
                AIAction action = action_list[o];
        ...
        private int CalculateActionHeuristics(Game data, AIAction action)
        {
            int playerID = original_data.current_player;
            ...
            NodeState action_node = CreateNode(first_node, action, playerID, first_node.tdepth, first_node.taction);

            int score = heuristic.CalculateHeuristic(ndata, action_node, game_logic);
        ...
        private bool isActionBeneficial(Game data, NodeState node, AIAction action)
        {
            action.score = CalculateActionHeuristics(data, action);
            return action.score > node.hvalue;
        }

        private void calculateActionScores(Game data, List<AIAction> action_list)
        {
            //Calculate scores
            for (int o = 0; o < action_list.Count; o++)
            {
        ...
        public AIAction getCurrentBestAction(Game data, List<AIAction> action_list)
        {
            AIAction currentBestAction = CreateAction(GameAction.EndTurn);
            int currentBestScore = first_node.hvalue;
            ...
            for (int o = 0; o < action_list.Count; o++)
            {
                ...
                if (action.valid)
                {
                    if (action.score > currentBestScore)
                    {
                        currentBestAction = action;
                        currentBestScore = action.score;
                    }
                }
            }
        ...
        private void CalculateChildNode(Game data, NodeState parent, AIAction action)
        {
            ...
            else
            {
                ...
                child_node.hvalue = heuristic.CalculateHeuristic(ndata, child_node, game_logic);
            }
        ...
        private void AddActions(List<AIAction> actions, Game data, NodeState node, ushort type, Card card, int boardZone = 0)
        {
            Player player = data.GetPlayer(data.current_player);
            ...
            if (card.HasStatus(StatusType.Paralyzed))
                return;
            ...
            if (type == GameAction.CastAbility)
            {
                ...
                for (int a = 0; a < abilities.Count; a++)
                {
                    ...
                    if (ability.trigger == AbilityTrigger.Activate && data.CanCastAbility(card, ability, boardZone) && ability.HasValidSelectTarget(data, card))
                    {
                        AIAction action = CreateAction(type, card);
        ...
        public AIAction GetBestLogicalAction()
        {
            return best_action;
        }
