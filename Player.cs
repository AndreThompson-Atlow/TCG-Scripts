...
namespace TcgEngine
{
    ...
    public class Player
    {
        ...
        public bool fullAction = false;
        public bool halfAction = false;
        ...
        public List<Card> cards_extra = new List<Card>();   //Cards in the player's extra deck
        ...
        public virtual void RemoveCardFromAllGroups(Card card, bool includeAllGroup = false)
        {
        ...
        public List<CardMythos> GetBoardMythos()
        {
            List<CardMythos> boardMythos = new List<CardMythos>();

            foreach (Card card in cards_board)
            {
                if (card.CardData.subtype == CardSubType.Summoner)
                {
                    boardMythos.Add(card.CardData.mythos);
                }
            }
        ...
        public Card GetExtraCard(string uid)
        {
            foreach (Card card in cards_extra)
            {
                if (card.uid == uid)
                    return card;
            }
        ...
        public virtual Slot GetRandomEmptySlot(System.Random rand, List<Slot> list_mem = null)
        {
            ...
            if (valid.Count > 0)
            {
                // Order slots based on proximity to the center (x = 3)
                return valid.OrderBy(slot => System.Math.Abs(slot.x - 3))
                            .FirstOrDefault();
            }
        ...
        {
            anyAction = fullAction || halfAction;
        }

        public void useAction(bool isFullAction = true)
        {
            if (isFullAction)
            {
                useFullAction();
        ...
        public void useFullAction()
        {
            if (bonusAction && fullAction)
            {
                fullAction = false;
                halfAction = true;
                anyAction = true;
        ...
        {
            anyAction = anyAction || halfAction || fullAction;
            return anyAction;
        }
        ...
        public virtual int GetStatusValue(StatusType effect)
        {
            ...
            int value1 = status1?.value ?? 0;
            int value2 = status2?.value ?? 0;
        ...
        {
            if (card.CardData.subtype == CardSubType.Skill)
            {
                return hp >= card.GetHPCost();
            }
            else
            {
        ...
        public static void Clone(Player source, Player dest)
        {
            ...
            dest.anyAction = source.anyAction;
            dest.fullAction = source.fullAction;
            dest.halfAction = source.halfAction;
            dest.bonusAction = source.bonusAction;
            ...
            Card.CloneListRef(dest.cards_all, source.cards_extra, dest.cards_extra);
